...
class ScheduledJobScanner(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> Any:
        ...
        if _is_sudo_mutation(node.func, context.sudo_vars, constants, self.superuser_names):
            self._add(
        ...
        elif _is_sudo_business_method_call(node.func, context.sudo_vars, constants, self.superuser_names):
            self._add(
            ...
            severity = (
                "medium" if _is_sudo_expr(node.func, context.sudo_vars, constants, self.superuser_names) else "low"
            )
            self._add(
        ...
        if _is_http_call(node.func, self.http_module_aliases, self.http_function_aliases, context.http_client_vars):
            ...
            for url_value in _http_url_values(node, sink, constants):
                ...
                if _literal_url_has_embedded_credentials(url_value, constants):
                    self._add(
                        "odoo-scheduled-job-url-embedded-credentials",
    ...
    def _record_alias_target(self, target: ast.AST, value: ast.AST, context: JobContext) -> None:
        self._track_alias(
            ...
            value,
            context.sudo_vars,
            lambda node: _is_sudo_expr(node, context.sudo_vars, self._effective_constants(), self.superuser_names),
        )
        self._track_alias(
            target,
            value,
            context.http_client_vars,
            lambda node: (
                _is_http_client_expr(node, self.http_module_aliases)

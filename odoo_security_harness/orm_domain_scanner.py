...
class OrmDomainScanner(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> Any:
        ...
        self.route_stack.append(
            _route_info(node, self._effective_constants(), self.route_names, self.http_module_names)
    ...
    def _is_lambda_with_env_or_request(self, node: ast.AST) -> bool:
        return _is_lambda_with_env_or_request(node, self.request_names)
...
def _is_lambda_with_env_or_request(node: ast.AST, request_names: set[str]) -> bool:
    ...
    return any(marker in text for marker in ("request.", "env.context", "_context", ".sudo(", ".with_user(")) or any(
        isinstance(child, ast.Name) and child.id in request_names for child in ast.walk(node.body)
    )

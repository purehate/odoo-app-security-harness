...
_TAXONOMY_SHAPE_HINTS = (
    ...
    (
        ...
        (
            ...
            "manifest-insecure-python-dependency",
            "manifest-floating-vcs-python-dependency",
            "manifest-local-python-dependency",
            "manifest-local-bin-dependency",
            ...
            "cleartext http:// package references",
            "vcs references without immutable commit pins",
    ...
    (
        ...
        (
            ...
            "web-owl-qweb-dangerous-url-scheme",
    ...
    (
        ...
        (
            "web-insecure-asset-url",
            "web-insecure-http-request-url",
            "web-owl-qweb-insecure-asset-url",
            "qweb-insecure-asset-url",
            "manifest-insecure-remote-asset",
            "manifest-protocol-relative-remote-asset",
            "mail-template-insecure-url",
            "i18n-insecure-url",
            "web-insecure-live-connection-url",
            ...
            "qweb template loads insecure http url",
            "owl inline template loads insecure http url",
            "manifest frontend assets reference cleartext http:// urls",
            "manifest frontend assets reference protocol-relative urls",
            "mail template contains insecure http url",
            "translation introduces insecure http url",
            "frontend browser request targets a literal http:// url",
            "literal http:// url in a link, frame, form, or media attribute",
            "literal http:// url in attribute",
            "loads a script over http://",
            "loads a stylesheet over http://",
            "literal cleartext ws:// or http:// url",
            "body_html contains a literal http:// url",
            "translated msgstr contains a literal http:// url",
    ...
    (
        ...
        (
            ...
            "web-owl-qweb-dynamic-class-attribute",
            "qweb dynamic class attribute",
            ...
            "web-owl-qweb-dynamic-style-attribute",
    ...
    (
        "file_upload_active_content_attachment",
        (
            "odoo-file-upload-active-content-attachment",
            "uploaded attachment uses browser-active content type",
            "ir.attachment.create stores uploaded/browser-active content",
            "mime allowlists, sanitization, download disposition, and public access",
        ),
    ),
    (
...
def _regex_route_decorator_names(text: str) -> set[str]:
    ...
    for clause in _regex_import_clauses(text, "odoo"):
    ...
    for clause in _regex_import_clauses(text, "odoo.http"):
        for part in clause.split(","):
            tokens = part.strip().split()
...
def _regex_import_clauses(text: str, module: str) -> list[str]:
    clauses: list[str] = []
    pattern = re.compile(
        rf"from\s+{re.escape(module)}\s+import\s+(?:\((?P<paren>.*?)\)|(?P<line>[^\n]+))",
        re.DOTALL,
    )
    for match in pattern.finditer(text):
        clause = match.group("paren") if match.group("paren") is not None else match.group("line")

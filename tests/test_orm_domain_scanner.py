...
def test_updated_public_route_options_tainted_search_is_high(tmp_path: Path) -> None:
    """Updated **route options should preserve public ORM-domain severity."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "main.py").write_text(
        """
...
ROUTE_OPTIONS = {'auth': 'user', 'route': '/internal/search'}
ROUTE_OPTIONS.update({'auth': 'none', 'route': '/public/search'})

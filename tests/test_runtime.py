...
""",
    ...
    def test_runner_fallback_extracts_routes_keyword_paths(self, tmp_path: Path) -> None:
        """Syntax-error fallback should preserve routes=[...] keyword paths."""
        namespace = runpy.run_path(str(RUN_SCRIPT), run_name="__test_odoo_run__")
        module_dir = tmp_path / "demo"
        controller_dir = module_dir / "controllers"
        controller_dir.mkdir(parents=True)
...
class Demo:
    @http.route(routes=['/demo/a', '/demo/b'], auth='public')
    def fallback(self):
        return 'ok'
...
""",
    ...
    def test_runner_fallback_extracts_multiline_imported_route_alias(self, tmp_path: Path) -> None:
        """Syntax-error fallback should preserve parenthesized odoo.http route imports."""
        namespace = runpy.run_path(str(RUN_SCRIPT), run_name="__test_odoo_run__")
        module_dir = tmp_path / "demo"
        controller_dir = module_dir / "controllers"
        controller_dir.mkdir(parents=True)
...
class Demo:
    @odoo_route('/demo/multiline-alias', auth='public')
    def fallback(self):
        return 'ok'
...
""",
        ...
        assert len(routes) == 1
        assert routes[0]["paths"] == ["/demo/multiline-alias"]
        assert routes[0]["auth"] == "'public'"

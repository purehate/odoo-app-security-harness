...
def test_flags_dict_union_static_unpack_public_portal_route_options(tmp_path: Path) -> None:
    """Dict-union route option unpacking should preserve public portal context."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "portal.py").write_text(
        """
...
def test_flags_updated_static_unpack_public_portal_route_options(tmp_path: Path) -> None:
    """Updated route option dictionaries should preserve public portal context."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "portal.py").write_text(
        """
...
""",
    ...
    assert "odoo-portal-public-route" in rule_ids
    assert "odoo-portal-access-token-without-helper" in rule_ids
    assert any(f.route == "/my/orders/<int:order_id>" for f in findings)

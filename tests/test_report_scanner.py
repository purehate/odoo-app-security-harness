...
def test_updated_static_unpack_public_report_render_is_reported(tmp_path: Path) -> None:
    """Updated **route options should not hide public report rendering."""
    controller = tmp_path / "module" / "controllers" / "report.py"
    controller.parent.mkdir(parents=True)
    controller.write_text(
        """
...
class Controller(http.Controller):
    ...
    def private(self, order_id):
        marker, *items, tail = (
            'x',
            request.env['sale.order'].sudo().browse(order_id),

...
def test_nested_static_unpack_public_bus_send_with_sensitive_payload(tmp_path: Path) -> None:
    """Nested static **route options should not hide public bus sends."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "bus.py").write_text(
        """
...
def test_updated_static_unpack_public_bus_send_with_sensitive_payload(tmp_path: Path) -> None:
    """Updated **route options should not hide public bus sends."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "bus.py").write_text(
        """
...
BUS_OPTIONS = {'route': '/internal/bus', 'auth': 'user'}
BUS_OPTIONS.update({'route': '/public/bus', 'auth': 'none'})

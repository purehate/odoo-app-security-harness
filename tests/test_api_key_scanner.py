...
def test_dict_union_static_unpack_route_options_api_key_create_is_reported(tmp_path: Path) -> None:
    """Dict-union ** route options should not hide public API-key mutations."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "apikey.py").write_text(
        """

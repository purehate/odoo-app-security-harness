...
def test_server_action_url_embedded_credentials(tmp_path: Path) -> None:
    """XML server actions should flag credentials embedded in outbound URLs."""
    xml = tmp_path / "actions.xml"
    xml.write_text(
        """<odoo>
  <record id="action_http_url_credentials" model="ir.actions.server">
    <field name="state">code</field>
    <field name="code"><![CDATA[
PARTNER_URL = 'https://integration_user:sk_live_1234567890abcdef@hooks.example.test/action'
HTTP_OPTIONS = {'url': 'https://token_1234567890abcdef@partner.example.test/action', 'timeout': 10}
...
requests.request('POST', **HTTP_OPTIONS)
    ...
    assert any(f.rule_id == "odoo-xml-server-action-url-embedded-credentials" for f in findings)
...
def test_cron_url_embedded_credentials(tmp_path: Path) -> None:
    """XML cron inline Python should flag credentials embedded in outbound URLs."""
    xml = tmp_path / "cron.xml"
    xml.write_text(
        """<odoo>
  <record id="cron_http_url_credentials" model="ir.cron">
    <field name="state">code</field>
    <field name="code"><![CDATA[
FEED_URL = 'https://integration_user:sk_live_1234567890abcdef@feeds.example.test/orders'
HTTP_OPTIONS = {'url': 'https://token_1234567890abcdef@partner.example.test/cron', 'timeout': 10}
requests.get(FEED_URL, timeout=10)
requests.request('POST', **HTTP_OPTIONS)
    ]]></field>
  </record>
    ...
    assert any(f.rule_id == "odoo-xml-cron-url-embedded-credentials" for f in findings)

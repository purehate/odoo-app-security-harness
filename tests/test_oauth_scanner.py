...
def test_oauth_http_dict_union_static_kwargs_timeout_is_not_reported(tmp_path: Path) -> None:
    """Dict-union static **kwargs should satisfy OAuth provider HTTP timeout checks."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "oauth.py").write_text(
        """
...
def test_oauth_http_verify_false_dict_union_static_kwargs_is_reported(tmp_path: Path) -> None:
    """Dict-union static **kwargs should not hide disabled OAuth HTTP TLS verification."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "oauth.py").write_text(
        """
...
def test_oauth_cleartext_http_url_dict_union_static_kwargs_is_reported(tmp_path: Path) -> None:
    """Dict-union static **kwargs should not hide cleartext OAuth provider URLs."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "oauth.py").write_text(
        """
...
BASE_OPTIONS = {'url': 'http://provider.example.test/oauth/token'}
TOKEN_OPTIONS = BASE_OPTIONS | {'timeout': 10}

class OAuthController(http.Controller):
    @http.route('/auth/oauth/callback', auth='public', csrf=False)
    def callback(self, **kwargs):
        return requests.request('POST', **TOKEN_OPTIONS)
""",
        encoding="utf-8",
    )
...
def test_dict_union_static_unpack_route_options_oauth_callback_risks_are_reported(tmp_path: Path) -> None:
    """Dict-union ** route options should not hide public OAuth callbacks."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "oauth.py").write_text(
        """
...
def test_unpacked_updated_oauth_identity_payload_is_reported(tmp_path: Path) -> None:
    """Unpacked dict.update maps should not hide OAuth identity payload writes."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "oauth.py").write_text(
        """
...
class Controller(http.Controller):
    ...
    def callback(self, **kwargs):
        Users = request.env['res.users'].with_user(1)
        vals = {}
        changes = {'oauth_uid': kwargs.get('sub'), 'login': kwargs.get('email')}
        vals.update(**changes)
        return Users.write(vals)
""",
        encoding="utf-8",
...
def test_unpacked_updated_token_payload_tainted_redirect_uri_is_reported(tmp_path: Path) -> None:
    """Unpacked dict.update token payloads should keep redirect_uri taint visible."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "oauth.py").write_text(
        """
...
class Controller(http.Controller):
    ...
    def callback(self, code, **kwargs):
        payload = {'grant_type': 'authorization_code', 'code': code}
        changes = {'redirect_uri': kwargs.get('redirect_uri')}

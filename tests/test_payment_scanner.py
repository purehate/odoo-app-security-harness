...
def test_updated_static_unpack_payment_callback_without_signature_is_reported(tmp_path: Path) -> None:
    """Updated route option dictionaries should not hide auth/csrf/path posture."""
    py = tmp_path / "controllers.py"
    py.write_text(
        """
from odoo import http
...
def test_payment_state_assignment_without_validation_is_reported(tmp_path: Path) -> None:
    """Direct state assignment should be treated like payment state transition helpers."""
    py = tmp_path / "models.py"
    py.write_text(
        """
from odoo import models

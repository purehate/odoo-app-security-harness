...
def test_taxonomy_coverage_classifies_attachment_metadata_specific_risks() -> None:
    ...
    coverage = odoo_deep_scan._taxonomy_coverage(
        [
            ...
            {
                "source": "attachments",
                "rule_id": "odoo-attachment-tainted-url",
                "title": "Attachment URL is request-controlled",
                "message": "ir.attachment.create stores a request-derived URL; validate allowed schemes, trusted hosts, portal visibility, and whether users can be sent to untrusted document links",
            },
            {
                "source": "attachments",
...
def test_taxonomy_coverage_classifies_outbound_integration_credential_forwarding() -> None:
    ...
    coverage = odoo_deep_scan._taxonomy_coverage(
        [
            ...
            {
                "source": "integrations",
                "rule_id": "odoo-integration-url-embedded-credentials",
                "title": "Outbound integration URL embeds credentials",
                "message": "Outbound HTTP URL embeds username, password, or token material in the URL authority",
            },
        ]
    )
    ...
    assert {entry["rule_id"]: entry["shape"] for entry in coverage["mapped_entries"]} == {
        ...
        "odoo-integration-url-embedded-credentials": "integration_url_embedded_credentials",
    }
    assert all("CWE-522" in entry["cwe"] for entry in coverage["mapped_entries"])
...
def test_taxonomy_coverage_classifies_ui_and_xml_privilege_rule_gaps() -> None:
    ...
    coverage = odoo_deep_scan._taxonomy_coverage(
        [
            ...
            {
                "source": "xml-data",
                "rule_id": "odoo-xml-mail-server-hardcoded-credential",
                "title": "XML mail server commits SMTP credentials",
                "message": "ir.mail_server data includes a literal SMTP password; move outbound mail credentials to deployment secrets or administrator-managed configuration",
            },
            {
                "source": "xml-data",

...
def test_flags_nested_static_unpack_public_route_options(tmp_path: Path) -> None:
    """Nested ** option dictionaries should preserve route posture checks."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "cors.py").write_text(
        """
...
def test_flags_updated_static_unpack_public_route_options(tmp_path: Path) -> None:
    """Updated route options should preserve route posture checks."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "cors.py").write_text(
        """
...
def test_flags_nested_static_unpack_csrf_and_methods(tmp_path: Path) -> None:
    """Nested static **options should preserve csrf and method posture checks."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "upload.py").write_text(
        """

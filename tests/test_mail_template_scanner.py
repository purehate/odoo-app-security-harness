...
def test_insecure_http_url_is_reported(tmp_path: Path) -> None:
    """Outbound email links should not downgrade users to cleartext HTTP."""
    xml = tmp_path / "mail.xml"
    xml.write_text(
        """<odoo>
  <record id="template_http_link" model="mail.template">
    <field name="body_html"><![CDATA[<a href="http://portal.example.com/pay">Pay</a>]]></field>
  </record>
</odoo>""",
        encoding="utf-8",
    )
...
def test_https_url_is_ignored_for_insecure_mail_url(tmp_path: Path) -> None:
    ...
    xml.write_text(
        """<odoo>
  <record id="template_https_link" model="mail.template">
    <field name="body_html"><![CDATA[<a href="https://portal.example.com/pay">Pay</a>]]></field>
  </record>
</odoo>""",
        encoding="utf-8",
    )
    ...
    assert not any(f.rule_id == "odoo-mail-template-insecure-url" for f in findings)

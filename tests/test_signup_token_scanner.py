...
def test_dict_union_static_unpack_route_options_public_reset_token_lifecycle_risks_are_reported(
    tmp_path: Path,
) -> None:
    """Dict-union ** route options must not hide public reset token risks."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "reset.py").write_text(
        """
...
def test_flags_updated_identity_token_write_values(tmp_path: Path) -> None:
    """dict.update calls should not hide token/password mutation dictionaries."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "reset.py").write_text(
        """
...
class Controller(http.Controller):
    ...
    def reset_password(self, **kwargs):
        Users = request.env['res.users'].sudo()
        values = {}
        values.update({'signup_token': kwargs.get('token'), 'password': kwargs.get('password')})
        return Users.write(values)
""",
        encoding="utf-8",
    )

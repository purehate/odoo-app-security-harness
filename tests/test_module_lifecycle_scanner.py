...
def test_dict_union_static_unpack_public_route_immediate_install_from_request_is_reported(tmp_path: Path) -> None:
    """Dict-union **route options should not hide public module lifecycle calls."""
    controllers = tmp_path / "module" / "controllers"
    controllers.mkdir(parents=True)
    (controllers / "modules.py").write_text(
        """
